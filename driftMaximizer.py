...
def Expected_increment_calculator(n, l, m, k): # forse il max_i che ci interessa dev'essere sempre n-l
    ...
    if l < n-1:
        if m == n-1:
            ...
            P_string[(0,)] = 1 - m/math.comb(n-l-1, m) # math.comb(n-(l+1), m-l)/math.comb(n-(l+2), m-l)
            
        # Tutta la cosa qui sotto, specie k == 1 mi pare strano
        if k > 1 and k <= n-l:
            ...
            for string in strings[i]: # We are considering strings 1...10string
                ...
                if string[-1] == 1: # I flip if l+i+1 is 1
                    ...
                    if P_flip[tuple(alt_string)] == 0:
                        ...
                        if k-num_zeros <= n-l-i:
                            Cond_p_flip = 0 # qui è zero o uno?
                        # elif num_zeros < k:
                        #     Cond_p_flip = 1 # Controllare che sia 1 e non 0
                        else:
                            Cond_p_flip = 1 - math.comb(n-l-i-1, k-num_zeros-1)/math.comb(n-l-i, k-num_zeros) # Non mi convince il < stretto
                        P_flip[tuple(string)] = Cond_p_flip * P_flip[tuple(alt_string)]
